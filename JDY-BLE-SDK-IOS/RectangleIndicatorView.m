...
- (void)updateUI {
    //━━━━━━━━━━━━━━━━━━━━ 中间的数值 ━━━━━━━━━━━━━━━━━━━━
    NSString *valueString = [NSString stringWithFormat:@"%@%@", @(_centerValue), @"  "];
        ...
        NSBlockOperation *operation_AddNewLayer2 = [NSBlockOperation blockOperationWithBlock:^{
            ...
            [mainQueue addOperationWithBlock:^{
                ...
                if (!self.isStop) {
                    ...
                    [self setCenterValue:(int)(fromValue + (minus > 0 ? i : -i)*((int)(fromValue-toValue)/(fromLineNumber - toLineNumber)==0 ? 2 : (fromValue-toValue)/(fromLineNumber - toLineNumber)))];
                } else {
                    //YYLog(@"遇到锁，取消执行");
                }
            }];
...
- (void)showAccessoryOnLineWitLineNumber:(NSInteger)lineNumber {
    ...
    indicatorLabel.text = [NSString stringWithFormat:@" "];
