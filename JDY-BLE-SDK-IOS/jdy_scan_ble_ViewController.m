...
- (void)viewDidLoad
{
    ...
    //    [t controlSetup:1];
    t.delegate = self;
    [tab setCellLayoutMarginsFollowReadableWidth:FALSE];
...
-(int)read_rssi_event
{
    ...
    return t.activePeripheral.RSSI.intValue;
    //        if(gn_view_ap!=nil){
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    strNSString =  [NSString stringWithFormat:@"Name:%@",[t.JDY_BLE_NAME objectAtIndex:row]];
    // [[NSString alloc] initWithUTF8String:[t UUIDToString:[aPeripheral UUID]]];
    
    mac = [t get_mac_address:(int)row];//[t.MAC_ADDRESS objectAtIndex:row];
    NSString *str=[NSString stringWithFormat:@"MAC:%@",mac];
    Byte ssd = [t get_jdy_ble_type:(int)row];
    
    //    strNSString = [NSString stringWithFormat:@"%02x",ssd ];
    //strNSString = [t get_ibeacon_major:(int)row];
    //    strNSString = [t get_ibeacon_minor:(int)row];
    //    strNSString = [t get_sensor_temp:(int)row];
    //    strNSString = [t get_sensor_humid:(int)row];
    //    strNSString = [t get_sensor_batt:(int)row];
    
    //NSLog(@"name===%@",[t.JDY_BLE_NAME objectAtIndex:row]);
    
    int issi1 =  [t get_rssi:(int)row];
    NSString *str_rssi = [NSString stringWithFormat:@"    RSSI:%d",issi1];
    
    NSString *ibeacon_text = @"";
    NSString *dev_type_string = @"";
    if( ssd==ibeacon )//   selected_type
    {
        dev_type_string = @"iBeacon设备";
        ibeacon_text = [NSString stringWithFormat:@"Major:%@   Minor:%@",[t get_ibeacon_major:(int)row],[t get_ibeacon_minor:(int)row] ];
        
        
    }
    else if( ssd==sensor_temp )
    {
        dev_type_string = @"温度传感器";
        ibeacon_text = [NSString stringWithFormat:@"温度:%@   湿度:%@   电量:%@",[t get_sensor_temp:(int)row],[t get_sensor_humid:(int)row],[t get_sensor_batt:(int)row]  ];
        
    }
    
    else if( ssd==touchuang ) {dev_type_string = @"透传";}
    else if( ssd==pwm_dentiao ) {dev_type_string = @"LED灯";cell.image.image = [UIImage imageNamed:@"light_semitrans.png"]; }
    else if( ssd==pwm_anmoqi_0 ) {dev_type_string = @"按摩棒";cell.image.image = [UIImage imageNamed:@"massager100.png"]; }
    else if( ssd==io_key4 ) {dev_type_string = @"四路开关控制器";cell.image.image = [UIImage imageNamed:@"switch_img.png"];  }
    else {dev_type_string = @"透传";  }
    ...
    dev_type_string = [NSString stringWithFormat:@"设备类型：%@,%@",dev_type_string,str_rssi ];
    
    
    
    cell.lableTitle.text = strNSString;
    cell.lableTime.text = str;
    cell.stat.text = dev_type_string;
    cell.ibeacon.text = ibeacon_text;
    //cell.image.image = [UIImage imageNamed:@"light_semitrans.png"];
    
    if( ssd==ibeacon )
    {
        cell.image.image = [UIImage imageNamed:@"tm_img.png"];
    }
    else if( ssd==sensor_temp )
    {
        cell.image.image = [UIImage imageNamed:@"tm_img.png"];
    }
    else if( ssd==touchuang ) {cell.image.image = [UIImage imageNamed:@"tm_img.png"];}
    else if( ssd==pwm_dentiao ) {cell.image.image = [UIImage imageNamed:@"light_semitrans.png"]; }
    else if( ssd==pwm_anmoqi_0 ) {cell.image.image = [UIImage imageNamed:@"massager100.png"]; }
    else if( ssd==io_key4 ) {cell.image.image = [UIImage imageNamed:@"switch_img.png"];  }
    else {cell.image.image = [UIImage imageNamed:@"tm_img.png"];  }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath;
{
    ...
    //int ic=(int)([t.foundPeripherals count]);
    aPeripheral8 = (CBPeripheral *)[t.foundPeripherals objectAtIndex:row];
    
    [t stop_scan_ble];
    selected_type = [t get_jdy_ble_type:(int)row];
    ...
    if (![t get_connected_status])//[[t activePeripheral] isConnected])
    {
        ...
        // [time_11 setFireDate:[NSDate distantPast]];
...
-(void) JDY_BLE_Ready {
    ...
    if(ivt==0)
    {
        ...
        {
//            ibeacon_view = [[iBeacon_ViewController alloc] init];
//            //        tabBarViewController_.delegate = self;
//            [self.navigationController pushViewController:ibeacon_view animated:YES];
        }
